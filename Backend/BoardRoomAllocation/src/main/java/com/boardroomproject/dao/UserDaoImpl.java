...
public class UserDaoImpl implements UserDao{
	...
	public void updateUser(User user) {
	     ...
	     jdbcTemplateObject.update(updateUser, user.getPassword(),user.getfName(),user.getlName(),user.getUserName(),user.getAddress(),user.getContact(),user.getDob(),Character.toString(user.getGender()),user.getType(),user.getUserId());
	}
	@Override
	...
	public List<User> getUserByLocation(int lid) {
		 return jdbcTemplateObject.query("select * from user where location = ? and isArchived = ?",new RowMapper<User>(){  
		        @Override
